...
const NotificationMessage game_macros_blink_start_100_100 = {
    .type = NotificationMessageTypeLedBlinkStart,
    .data.led_blink.color = 0,
    .data.led_blink.on_time = 100,
    .data.led_blink.period = 120,
};

static const NotificationSequence game_macros_sequence_hw_blink = {
    &game_macros_blink_start_100_100,
    &message_blink_set_color_magenta,
    &message_do_not_reset,
    NULL,
};

static const NotificationSequence game_macros_sequence_hw_blink_stop = {
    &message_blink_stop,
    NULL,
};

static void
    game_macros_view_draw_arrow(Canvas* canvas, uint8_t x, uint8_t y, CanvasDirection dir) {
...
static void game_macros_view_control_draw(Canvas* canvas, void* model_raw) {
    ...
    elements_multiline_text_aligned(
        canvas, 15, 8, AlignLeft, AlignTop, furi_string_get_cstr(model->debug_string));
#ifdef GAME_MACROS_BLE
    const char* is_connected;
    if(furi_hal_bt_is_connected()) {
        is_connected = "Connected: OK";
    } else {
        is_connected = "Back to\n setup BT";
    }
    elements_multiline_text_aligned(canvas, 2, 17, AlignLeft, AlignTop, is_connected);
...
int32_t game_macros_view_control_input_play(void* context) {
    ...
    for(size_t i = 0; i < ctx->count; i++) {
        ...
        if(instruction->delay) {
            ...
            if(furi_thread_flags_wait(
                   GameMacrosThreadTerminate, FuriFlagWaitAny, instruction->delay_ms) &
               GameMacrosThreadTerminate) {
                return 0;
            }
            continue;
        }
        if(instruction->key.press) {
            notification_message(ctx->notifications, &game_macros_sequence_hw_blink);
        } else {
            notification_message(ctx->notifications, &game_macros_sequence_hw_blink_stop);
        }
...
void game_macros_view_control_thread_alloc(
    const char* name,
    size_t stack_size,
    GameMacrosControlThreadCtx* ctx) {
...
void game_macros_view_control_reset(GameMacrosViewControl* control){with_view_model(
    control->view,
    GameMacrosViewControlModel* model,
    { memset(model->map, 0, sizeof(GameMacrosScriptMapping)); },
    true)} // ViewDispatcherMessageTypeStop
